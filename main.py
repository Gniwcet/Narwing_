...
def who_has_won(host_choice, host_mention, opponent_choice, opponent_mention, game_mode, host_id, opponent_id=814170233659981904):
    ...
    if game_mode == "vs":
        if host_choice == opponent_choice:
            return "Egalité ! la partie va recommencer...", None, None
        elif host_choice == "🤚" and opponent_choice == "👊":
            return f"🎉 {host_mention} a gagné ! 🎉", host_id, opponent_id
        elif host_choice == "👊" and opponent_choice == "✌":
            return f"🎉 {host_mention} a gagné ! 🎉", host_id, opponent_id
        elif host_choice == "✌" and opponent_choice == "🤚":
            return f"🎉 {host_mention} a gagné ! 🎉", host_id, opponent_id
        else:
            return f"🎉 {opponent_mention} a gagné ! 🎉", opponent_id, host_id
    elif game_mode == "solo":
        if host_choice == opponent_choice:
            return "Egalité ! la partie va recommencer...", None, None
        elif host_choice == "🤚" and opponent_choice == "👊":
            ...
            return f"Désolé, j'ai gagné :)", opponent_id, host_id
...
# Fun Msg
    ...
    if message.content.endswith('oue'):
        bot_msg = await channel.send("STERN !")
    if message.content.endswith('oué'):
        bot_msg = await channel.send("STERN !")
    if message.content.endswith('ué'):
        bot_msg = await channel.send("STERN !")
    if message.content.endswith('ouai'):
        bot_msg = await channel.send("STERN !")
    if message.content.endswith('wai'):
        bot_msg = await channel.send("STERN !")
    if message.content.endswith('wé'):
        bot_msg = await channel.send("STERN !")
...
# ANNONCE !
    ...
    if message.content.startswith('!sh vs'):
        try:
            ...
            embed = discord.Embed(title="Désolé, vous devez mentionner quelqu'un.", description="**Réessayez.**")
            embed.set_footer(text=f"Host : {host}", icon_url=host.avatar_url)
            bot_msg = await channel.send(embed=embed)

        if "@" not in opponent_mention:
            embed = discord.Embed(title="Désolé, ce pseudo est invalide.", description="**Réessayez.**")
            embed.set_footer(text=f"Host : {host}", icon_url=host.avatar_url)
            bot_msg = await channel.send(embed=embed)

        elif opponent_mention == host_mention or opponent_mention == f"<@!{client.user.id}>":
            embed = discord.Embed(title="Désolé, vous ne pouvez pas vous battre vous-même ou contre moi.", description="**Réessayez.**")
            embed.set_footer(text=f"Host : {host}", icon_url=host.avatar_url)
            bot_msg = await channel.send(embed=embed)
            ...
            timeout = 10
            timer = 0
            embed = discord.Embed(title=f"Invitation d'un match de Shifumi !", description=f"**{opponent}, accepte le match avec : ✅, refuse avec : ❌**\nVous avez encore **{timeout} secondes**.")
            embed.set_footer(text=f"Host : {host}", icon_url=host.avatar_url)
            bot_msg = await channel.send(embed=embed)
            for emoji in YES_NO:
            ...
            while timeout != 0:
                ...
                timeout -= 1
                if timeout == 0:
                    embed = discord.Embed(title=f"Invitation d'un match de Shifumi!", description="L'invitation à expiré.")
                    embed.set_footer(text=f"Host : {host}", icon_url=host.avatar_url)
            ...
            if game_continue is False:
                await bot_msg.clear_reactions()
                embed = discord.Embed(title=f"Invitation Refusée !")
                embed.set_footer(text=f"Host : {host}", icon_url=host.avatar_url)
                await bot_msg.edit(embed=embed)
                return
                ...
                while not game_end:
                    ...
                    timer = 0
                    embed = discord.Embed(title=f"Invitation Acceptée !", description=f"**La partie va commencer...**")
                    embed.set_footer(text=f"Host : {host}", icon_url=host.avatar_url)
                    await bot_msg.edit(embed=embed)
                    while decount_until_start != 0:
                        ...
                        embed = discord.Embed(title=f"Shifumi Match !", description=f"**Choisi la réaction que tu veux quand le compteur est égal à 0\n🤚 = Feuille\n👊 = Pierre\n✌ = Ciseaux\n**")
                        ...
                        if decount_until_start == 1:
                            ...
                            if out_of_time:
                                await bot_msg.clear_reactions()
                                embed = discord.Embed(title=f"Le match de Shifumi a Expiré", description=f"Vous avez mis trop de temps... Réessayez.")
                                embed.set_footer(text=f"Host : {host}", icon_url=host.avatar_url)
                                await bot_msg.edit(embed=embed)
                                return
                                ...
                                embed = discord.Embed(title=f"Les résultats du match de Shifumi", description=f"{result}\n------------------------------\n{host_mention} : {host_choice}\n{opponent_mention} : {opponent_choice}")
                                embed.set_footer(text=f"Host : {host}", icon_url=host.avatar_url)
                                await bot_msg.edit(embed=embed)
                          
                                #On ajoute les résutats si il n'y a pas égalité
        ...
        timeout = 5
        while timeout >= 0:
            embed = discord.Embed(title=f"Match solo de Shifumi", description=f"{host_mention}, Etes vous prêt ?\nLa partie commence dans **{timeout} secondes**")
        ...
        while not game_end:
            ...
            while decount_until_start != 0:
                ...
                if decount_until_start > 1:
                    ...
                    indications = ":red_circle: GO ! :red_circle:"
                embed = discord.Embed(title=f"Shifumi Match !",description=f"**Choisi la réaction que tu veux quand le compteur est égal 0\n🤚 = Feuille\n👊 = Pierre\n✌ = Ciseaux\n**")
                embed.add_field(name=f"Counter : {decount_until_start}", value=f"**{indications}**")
                embed.set_footer(text=f"Player : {host}", icon_url=host.avatar_url)
                await bot_msg.edit(embed=embed)
                    ...
                    try:
                        ...
                        if "100%winrate" in [role.name for role in player.roles]:
                            if str(host_choice) == "🤚":
                                ...
                                bot_choice = "🤚"
                        #Si un utilisateur a un role nommé "100%winrate, il gagnera à chaque fois"
                        ...
                        #On ajoute les résultats à la database si il n'a pas égalité
                        if winner_id != None and loser_id != None:
                            add_result_to_classement(winner_id, loser_id)
